# Daftar master: pencarian & filter (list search & filters)

## Purpose

Menyediakan **parameter query opsional** pada endpoint `GET` daftar resource master untuk **pencarian teks** (`q`) dan **filter facet**, dengan logika **AND**, tanpa mengubah bentuk respons `{ items: [...] }` pada MVP.
